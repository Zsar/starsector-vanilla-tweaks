...
public class DisintegratorEffect extends BaseCombatLayeredRenderingPlugin implements OnHitEffectPlugin {
	...
	private boolean targetIsAlive() {
		return (this.target instanceof ShipAPI ship)
		     ? CombatEngine.getInstance().isShipAlive(ship) || ship.isHulk()
		     : !target.isExpired() && !target.wasRemoved() && target.getHitpoints() > 0.f;
	}

	public void onHit(DamagingProjectileAPI projectile, CombatEntityAPI target, Vector2f point, boolean shieldHit, ApplyDamageResultAPI damageResult, CombatEngineAPI engine) {
		if (shieldHit) return;
		if (projectile.isFading()) return;
		if (!(target instanceof CombatAsteroidAPI || target instanceof ShipAPI)) return;

		Vector2f offset = Vector2f.sub(point, target.getLocation(), new Vector2f());
		offset = Misc.rotateAroundOrigin(offset, -target.getFacing());

		DisintegratorEffect effect = new DisintegratorEffect(projectile, target, offset);
	...
	public DisintegratorEffect(DamagingProjectileAPI proj, CombatEntityAPI target, Vector2f offset) {
		this.proj = proj;
		this.target = target;
		this.offset = offset;
	...
	public void advance(float amount) {
		...
		float volume = 1f;
		if (ticks >= getNumTicks() || !this.targetIsAlive() || !Global.getCombatEngine().isEntityInPlay(target)) {
			fader.fadeOut();
			fader.advance(amount);
	...
	protected void dealDamage() {
		...
		final float damagePerTick = (float) getTotalDamage() / (float) getNumTicks();
		...
		if (target instanceof ShipAPI ship) {
			...
			float damageTypeMult = getDamageTypeMult(proj.getSource(), ship);
			...
			for (int i = -2; i <= 2; i++) {
				for (int j = -2; j <= 2; j++) {
					...
					grid.setArmorValue(cx, cy, Math.max(0, armorInCell - damage));
					damageDealt += damage;
				}
			}

			if (damageDealt > 0) {
				if (Misc.shouldShowDamageFloaty(proj.getSource(), ship)) {
			...
			if (hullDamage > 1f) {
				...
				if (showHullDamage >= 0) {
					...
					if (target.getHitpoints() <= 0f && !ship.isHulk()) {
...
//				System.out.println("Total hull damage dealt: " + total);
			...
			damageDealt = damagePerTick * (1 / 15f);
			hullDamage = damageDealt;
			target.setHitpoints(target.getHitpoints() - hullDamage);
			if (target.getHitpoints() <= 0.f)
				engine.applyDamage(target, point, 100f, DamageType.ENERGY, 0f, true, false, proj.getSource(), false);
		}
	...
	public boolean isExpired() {
		return particles.isEmpty() &&
					(ticks >= getNumTicks() || !this.targetIsAlive() || !Global.getCombatEngine().isEntityInPlay(target));
	}
